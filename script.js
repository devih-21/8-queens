...
console.log("By Devih From PTIT");

let checkResult = (x2,y2) => {
    for(let i = 1; i < x2 ;i++)
        if(checkAllowPositionQueen[i] == y2 || Math.abs(i-x2) == Math.abs(checkAllowPositionQueen[i] - y2) )
            return false;
    return true;
}
...
let returnResult = (sizeChessBoard) => {
    ...
    for(let i = 1; i <= sizeChessBoard; i++){
        tempResult.push(`${checkAllowPositionQueen[i]}${i}`)
    }
